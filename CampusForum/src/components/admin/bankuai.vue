<template>
    <el-config-provider :locale="locale">
      ...
      <div style="display: flex; align-items: center; margin-bottom: 15px">
        <el-button plain type="primary" size="large" @click="AddAdminDia=true">添加板块</el-button>
      </div>
      <!-- 表格 -->
      <el-table :data="tableData" style="width: 100%" :table-layout="'auto'" class="etable">
        <el-table-column prop="id" label="板块id" fixed="left" />
    ...
    <el-dialog
        ...
        "
      >
        <el-input class="addipt" v-model="AdminObject.name" placeholder="请输入板块名称"/>
  ...
  export default {
    ...
    methods: {
      ...
      //删除
      deleteUser() {
        request.delete('/category/' + this.delRepId).then((res) => {
          if (res) {
            alert('删除成功')
    ...
    addAdminFun(){
      request.post("/category",this.AdminObject).then((res)=>{
          if(res){
            ElMessage("添加成功")
